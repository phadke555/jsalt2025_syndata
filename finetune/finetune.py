# PATCHED VERSION of f5_tts.train.finetuning_cli.py
# works for my specific custom dataset creation use case
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to YAML config file with finetuning options"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional base directory for your datasets. "
            "If set, will load `dataset_name` from this folder instead of the built-in data/"
        )
    )
...
def main():
    ...
    # PATCH
    # works for custom data saved to a custom data root
    if args.data_root:
        data_dir = os.path.join(args.data_root, args.dataset_name)
    else:
        data_dir = os.path.abspath(
            files("f5_tts").joinpath(f"../../data/{args.dataset_name}")
        )
    ...
    # with current usage will have to pass in a custom tokenizer path with my new dataset
    if tokenizer == "custom":
