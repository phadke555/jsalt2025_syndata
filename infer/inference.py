...
parser.add_argument(
    "--random_ref",
    action="store_true",
    help="Ignore --ref_audio/--ref_text and sample a random train example instead",
)
...
default_ref_audio = config.get("ref_audio", "infer/examples/basic/basic_ref_en.wav")
default_ref_text  = config.get("ref_text",  "Some call me nature…")

# if user wants a random reference from the train set:
if args.random_ref:
    if not args.data_root:
        raise ValueError("When using --random_ref you must also pass --data_root")
    meta_csv = os.path.join(args.data_root, "metadata.csv")
    ...
    # fall back on your existing args or config values:
    ref_audio = args.ref_audio or config.get("ref_audio", default_ref_audio)
    ref_text  = args.ref_text  if args.ref_text is not None else config.get("ref_text", default_ref_text)
...
def main():
    ...
    if generated_audio_segments:
        ...
        ref_dest = Path(output_dir) / f"ref_{ref_basename}"
        shutil.copy2(ref_audio, ref_dest)
        print(f"Reference audio saved to {ref_dest}")
